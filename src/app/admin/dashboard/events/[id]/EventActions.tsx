...
import { boardReview, treasurerReview, submitProposal, publishEvent, unpublishEvent, cancelEvent } from '../actions'
...
export default function EventActions({
  ...
  if (!canBoard && !canTreasurer && !canSubmit && !canEdit && !canPublish && !canUnpublish && !canCancel) {
    return (
    ...
    <div className="space-y-6">
      ...
      {(canPublish || canUnpublish || canCancel) && (
        <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-6">
          <h3 className="font-serif text-lg font-bold text-green">Manage</h3>

          {canPublish && (
            <form action={run(publishEvent)}>
              <input type="hidden" name="eventId" value={eventId} />
              <p className="mb-2 text-sm text-slate-600">
                Publishing makes it visible on the homepage and Events page, and lets you send RSVP invites.
              </p>
          ...
          {canUnpublish && (
            <form action={run(unpublishEvent)}>
              <input type="hidden" name="eventId" value={eventId} />
              <p className="mb-2 text-sm text-slate-600">
                Take it off the website and put it back on hold (you can re-publish later).
              </p>
              <Button type="submit" variant="ghost" disabled={pending}>Unpublish (move to Hold)</Button>
            </form>
          )}

          {canCancel && (
            <form action={run(cancelEvent)} className="border-t border-slate-100 pt-3">
              <input type="hidden" name="eventId" value={eventId} />
              <p className="mb-2 text-sm text-slate-600">Cancel this event and remove it from the website.</p>
              <button
                type="submit"
                disabled={pending}
                onClick={(e) => {
                  if (!window.confirm('Cancel this event? It will be removed from the website. This cannot be undone.')) {
                    e.preventDefault()
                  }
                }}
                className="rounded-md border border-red-300 px-4 py-2 font-semibold text-red-700 transition-colors hover:bg-red-600 hover:text-white disabled:opacity-50"
              >
                Cancel event
              </button>
            </form>
