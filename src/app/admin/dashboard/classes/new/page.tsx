...
export default async function CreateClassPage() {
  ...
  async function createClass(formData: FormData) {
    ...
    redirect(`/admin/dashboard/classes/${data.id}`)
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Define a new class, select the program type, and assign a teacher.
          </p>
        </div>
      </div>

      <form action={createClass} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="number" name="capacity" min="0" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-ink" placeholder="Leave blank for unlimited" />
            ...
            <div className="flex flex-wrap gap-3">
              {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => (
                <label key={day} className="flex items-center space-x-2 bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg cursor-pointer hover:bg-slate-100">
                  <input type="checkbox" name="days" value={day} className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 border-gray-300" />
